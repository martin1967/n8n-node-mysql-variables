...
(куки, API-ключи, client id/secret и любые другие значения) в **MySQL**.
...
| Host / Port / Database / User / Password | Подключение к MySQL (свой логин у каждого). |
| SSL | TLS-подключение (self-signed принимается). |
